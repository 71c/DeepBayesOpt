...
class SubmitTrainUtils(ABC):
    ...
    special_args: ClassVar[set[str]] = set()
    ...
    def get_cmd_opts_dataset(self, options: dict) -> dict:
        """Given command options in the form of a dict, return a dict which contains
        only those options that are related to dataset generation, not training."""
        pass

    def determine_whether_trained(self, options: dict, cmd_opts_nn: dict) -> bool:
        """This is the default implementation. To also be able to train baseline
        methods, subclasses can override to add such functionality"""
        ...
        return basic_save_utils.model_is_trained(model_and_info_name)

    def get_train_cmd_options(self, options, cmd_opts_dataset, cmd_args_dataset):
        """This is the default implementation. To also be able to train baseline
        methods, subclasses can override to add such functionality"""
        # Get NN training argument structure
        _, nn_arg_groups = self.torch_model_save_instance.get_single_train_parser_and_info()
        ...
        cmd_opts_nn_no_dataset = {
            ...
            if k not in self.special_args  # already in dataset
        }
    ...
    def get_dataset_and_train_cmd_options(self, options: dict[str, Any]):
    ...
    def create_submit_train_dependency_structure(
        ...
        for options, depdendent_commands in zip(options_list, dependents_list):
            ...
            cmd_nn_train, cmd_opts_nn) = self.get_dataset_and_train_cmd_options(options)
            ...
            if train_nn:
                ...
                if cmd_dataset in datasets_cached_dict:
                    ...
                    whether_cached = self.create_datasets_func(
    ...
    def submit_jobs(self, cmd_args: Optional[Sequence[str]] = None):
        ...
        train_base_config_name, train_experiment_config_name = add_submit_train_args(parser, train=True)

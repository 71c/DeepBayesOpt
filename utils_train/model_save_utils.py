...
from dataset.acquisition_dataset_manager import FIX_TRAIN_ACQUISITION_DATASET, get_lamda_min_max
from utils_general.dataset_with_models import RandomModelSampler
from dataset.gp_acquisition_dataset_manager import GP_GEN_DEVICE
from dataset.hpob_dataset import get_hpob_dataset_dimension
import torch
from utils.basic_model_save_utils import BASIC_SAVING
from utils.constants import MODELS_VERSION
...
from utils_general.training.train_utils import GI_NORMALIZATIONS
...
class AcquisitionFunctionNetSaveUtils(TorchModuleSaveUtils):
    ...
    @classmethod
    def add_single_train_args_and_return_info(cls, parser: argparse.ArgumentParser) -> Any:
        ################################ Dataset settings ##################################
    ...
    @classmethod
    def initialize_module_from_args(cls, args: argparse.Namespace) -> AcquisitionFunctionNet:
...
ACQF_NET_SAVING = AcquisitionFunctionNetSaveUtils(BASIC_SAVING)
